...
<!-- 
eu mudei o formado para esse rmdformats porque o html fica com uma lateral de sumário que facilita a navegação. para fazer o html tem que instalar o pacote rmdformats.
só não consegui fzer as figuras ficarem muito bonitas, mas dá pra clicar e ver cm mais zoom

abaixo as possibilidades do estilo anterior:
## New Styles
...
[^5]: caso queira ter acesso ao arquivo .Rmd do roteiro acesse o [repositório no github](https://github.com/melina-leite/melina-leite.github.io).

Nesse momento, você deve estar se perguntando: esse roteiro é para mim?! Para acompanha-lo é interessante ter uma compreensão básica de análises estatísticas em **R**. Isso quer dizer que você já usou e sabe interpretar o output de funções como `lm()`. Esperamos que ao terminar você seja capaz de: 
...
Nesta figura, temos as retas do modelo entre `Riqueza` e `NAP` aplicado para cada praia (usando a função `lm()`). Entretanto, nossa pergunta inicial não diz respeito à cada praia separadamente, nós queremos modelar esta relação para todas as praias amostradas, sem pseudoreplicação (não-independências dos dados) e queremos fazer isso de forma a ser possível predizer a riqueza em praias não amostradas.

Portanto, precisamos de um modelo linear que incorpore o fato de que nossos dados estão agrupados em praias. Chamamos de um **efeito aleatório** uma variável que agrupa nossos dados e que seu efeito sobre a variável resposta não nos interessa diretamente (nesse exemplo não interessa, mas depenendo da análise, pode interessar - veja discussão em McGill 2015). Nesse caso, os autores amostraram nestas 9 praias, mas poderiam ter feito o mesmo estudo escolhendo outras praias. O efeito aleatório praia "organiza" a parte da variação nos nossos dados que não conseguimos explicar, presente no erro $\varepsilon$ do modelo (Winter, 2013). Por outro lado, as variáveis preditoras que estamos acostumados a encontrar em modelos lineares são chamadas de **efeitos fixos**. No exemplo das praias, a variável `NAP` é um efeito fixo e estamos diretamente interessados em seu efeito sobre a variável resposta. O nome "misto" advém do fato de que existe ao menos um efeito fixo e um efeito aleatório. 
...
Nossa tabela da dados não possui nenhum valor faltante, mas caso possuísse, não tem muito problema em análises de modelos mistos. Agora, vamos construir nosso modelo.  
...
Relembrando, estamos interessados em ver se existe um efeito de `NAP` na riqueza de espécies. Nossos dados contam com nove diferentes praias, e cinco diferentes amostras para cada uma delas. Assim, temos como efeito fixo a variável `NAP`, e a variável aleatória praia (`Beach`), que significa que cada praia terá um intercepto diferente. Assim, nosso modelo é:
...
Existem, entretanto, diversas maneiras de criar seu modelo, escolher os parâmetros importantes, e decidir quais são fixos e quais são aleatórios. Nesse exemplo, poderíamos colocar a variável `Exposure` também como um efeito fixo (mais indicado seria usá-la como um fator com 3 níveis). Outra complicação que poderíamos inserir no noso modelo é inserir mais um efeito aleatório de interação entre a `praia` (efeito aleatório) e `NAP` (efeito fixo), fazendo com que cada praia possa ter inclinações de reta diferentes.

Por isso, é importante testas diferentes modelos antes de decidir qual o melhor. Na próxima parte abordaremoso como selecionar o melhor modelo dada todas as possibilidades. 
...
Para modelos que model ter mais de um efeito aleatório, Zuur et al. (2009), sugere um protocolo para a escolha da melhor estrutura de efeitos aleatórios. Vamos aos passos:
